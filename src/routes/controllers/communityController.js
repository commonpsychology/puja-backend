  ...
  try {
    ...
    let query = supabase
      ...
      .select(`
        ...
        spots_left, is_full, group_id, created_at,
        community_groups ( id, name, emoji, color )
      `)
...
// ─── RESERVE SESSION ─────────────────────────────────────────────────────────
const reserveSession = async (req, res, next) => {
  try {
    const { id } = req.params
    ...
    const {
      ...
      payment_status, payment_amount, payment_id,
    } = req.body

    // 1. check session exists + has spots
    const { data: session, error: sessErr } = await supabase
      .from('group_sessions')
      .select('id, max_spots, reserved_count, is_full, price')
      .eq('id', id)
      .single()

    if (sessErr || !session) return res.status(404).json({ message: 'Session not found' })
    if (session.is_full)     return res.status(400).json({ message: 'Session is full' })
    ...
    if (existing) return res.status(400).json({ message: 'You already have a reservation for this session' })
    ...
    await supabase.rpc('increment_session_reserved_count', { session_id: id })
...
// ─── CANCEL RESERVATION ──────────────────────────────────────────────────────
// handles both:
//   DELETE /community/sessions/:id/cancel-reservation
//   DELETE /community/reservations/:reservationId
const cancelReservation = async (req, res, next) => {
  try {
    const userId = req.user?.id
    if (!userId) return res.status(401).json({ message: 'Login required' })

    // support both route param styles
    ...
    if (fetchErr || !reservation) return res.status(404).json({ message: 'Reservation not found' })
    if (reservation.user_id !== userId) return res.status(403).json({ message: 'Forbidden' })

    await supabase.from('group_reservations').delete().eq('id', reservationId)
    await supabase.rpc('decrement_session_reserved_count', { session_id: reservation.session_id })
...
const myReservations = async (req, res, next) => {
  try {
    ...
    const { data, error } = await supabase
      .from('group_reservations')
      .select(`
        id, session_id, display_name, is_anonymous,
        payment_method, payment_reference,
        payment_status, payment_amount, payment_id, created_at,
        group_sessions (
          id, title, facilitator, scheduled_at, mode, price,
          community_groups ( id, name, emoji )
        )
      `)
      .eq('user_id', userId)
