...
//   1. PUT /orders/:id/assign-rider moved ABOVE PUT /orders/:id to prevent route swallowing
//   2. All delivery-rider routes included and ordered correctly
//   3. No duplicate route handlers
//   4. All imports present and correct
//   5. Debug routes kept (can be removed before production)
...
// ─── Orders ───────────────────────────────────────────────────
// IMPORTANT: specific sub-routes (/assign-rider, /status) MUST come
// before the generic /:id route — Express matches top-to-bottom and
// PUT /orders/:id would swallow /orders/:id/assign-rider otherwise.

router.get   ('/orders',            guard, getAllOrders)
router.patch ('/orders/:id/status', guard, setOrderStatus)

// ✅ FIX: assign-rider route BEFORE the generic PUT /orders/:id
router.put('/orders/:id/assign-rider', guard, async (req, res, next) => {
  try {
    ...
    // Only set delivery_status to 'assigned' if the order wasn't already
    // further along the delivery pipeline (don't regress from in_transit → assigned)
    if (rider_id) {
      ...
      if (existing && LOCK_STATUSES.includes(existing.delivery_status)) {
        // Just update the rider, don't change delivery_status
        delete updates.delivery_status
      }
    }
...
// Generic order status update — AFTER specific routes
...
router.put   ('/group-memberships/:id', guard, adminUpdateMembership)
router.delete('/group-memberships/:id', guard, adminDeleteMembership)
...
router.post('/course-playlists', guard, async (req, res, next) => {
  try {
    ...
    if (access_pin?.trim()) {
      ...
      payload.access_pin = hErr ? access_pin.trim() : hashed
    }
...
router.post('/delivery-riders', guard, async (req, res, next) => {
  ...
  try {
    const { full_name, email, phone, password, vehicle_type, vehicle_number, area, notes } = req.body
    ...
    // ── Create Supabase Auth user (same UUID) ─────────────────
