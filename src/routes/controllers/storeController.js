...
const listProducts = async (req, res) => {
  ...
  let query = supabase
    .from('products')
    .select('id, name, slug, short_description, price, sale_price, images, is_featured, category_id, stock_quantity', { count: 'exact' })
    .eq('is_active', true)
...
const createOrder = async (req, res) => {
  ...
  const { data: cartItems, error: cartError } = await supabase
    .from('cart_items')
    .select(`quantity, products:product_id(id, name, price, sale_price, stock_quantity), product_variants:variant_id(id, price)`)
    .eq('user_id', req.user.sub)
  ...
  if (orderError) return res.status(500).json({ success: false, message: 'Could not create order.' })

  // Insert line items
  const itemsWithOrderId = lineItems.map((item) => ({ ...item, order_id: order.id }))
